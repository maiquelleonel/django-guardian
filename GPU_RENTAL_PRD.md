...
## 2. Core Architectural Mandates (The Django Warden SLA)

This project must be designed from the ground up to comply with **django-warden**'s core principles:

1. **Thin Views & Lean Models (Anti-God Object):**
   - Views handle only HTTP, serialization, and schema validation.
   - Models (`GPUInstance`, `RentalLease`, `MetricSnapshot`, `UserCredit`, `TokenUsage`) only store data structure and internal invariants.
2. **Services vs. Orchestrators Separation (SOLID):**
   - `services/` handles third-party integrations (e.g., interfacing with the local payment mock, simulated notifications).
   - `orchestrators/` manages local business workflows, coordinating lease state machine transitions, upgrades, and billing rules.
3. **No Naive Datetimes:** Every timestamp uses `django.utils.timezone.now()`.
4. **No Blocked Threads:** All external client/HTTP communications (including payments mock calls) have strict `timeout` configurations to satisfy `warden.W006`.
5. **No Signal Windmills:** Senders and receivers are protected from infinite save recursion.
...
- **Quality Watchdog:** `django-warden` (pre-installed as a startup check)
...
1. **Setup Environment:**
   ...
   * Install `django-warden` and add `"django_warden"` to `INSTALLED_APPS`.
2. **First Run (`warden_audit`):**
   * Run `python manage.py check` to verify that `django-warden` checks pass and recommend packages.
   * Run `python manage.py warden_audit` to see the initial architectural compliance score.
...
6. **Verify and Audit:**
   * Keep running `python manage.py warden_audit` during development to ensure zero architectural degradation.
